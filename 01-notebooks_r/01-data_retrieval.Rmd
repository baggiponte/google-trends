...
Sometimes, `hits` returns a string: `"<1"`. We need to handle this and avoid getting 0s - because it will hinder our scaling.

```{r}
gtrends("Mario Draghi", time = "2020-11-02 2021-07-29")$interest_over_time %>% 
  mutate(
    hits = case_when(
      hits == "<1" ~ "1",
...
# Retrieve the Series for the Keywords of Interest

```{r}
keywords = c("Mario Draghi", "Euro", "Christine Lagarde", "European Central Bank")
start = "2012-01-01"
end = "2021-08-28"

for (keyword in keywords) {
  print(glue::glue("Retrieving Series for keyword {keyword}"))
  assign(snakecase::to_snake_case(keyword), retrieve_daily_data(start = start, end = end, keyword = keyword))
}
```
...
```{r}
mario_draghi %>% 
  bind_rows(christine_lagarde) %>% 
  bind_rows(euro) %>% 
  bind_rows(european_central_bank) -> final_dataset

final_dataset %>% write_csv(here("data", "keywords.csv"))
```
